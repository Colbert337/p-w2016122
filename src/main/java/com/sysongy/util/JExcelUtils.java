...
public class JExcelUtils {
	...
	public void createExcelFile(String path, String sheetName,
			String[] dataTitles, String titleName, String logoPath) {
		...
		try {
			...
			/*
			 * 报表主标题 合并单元格 通过writablesheet.mergeCells(int x,int y,int m,int
			 * n);来实现的 表示将从第x+1列，y+1行到m+1列，n+1行合并
			 */
			sheet.mergeCells(0, 0, dataTitles.length - 1, 0);
			label = new Label(0, 0, titleName, getDataCellFormat(
					CellType.STRING_FORMULA, "宋体", 20, Colour.BLUE,
					Colour.YELLOW));
			sheet.addCell(label);
			...
			for (int i = 0; i < data.size(); i++) {
				...
				insertOneCellData(sheet, 0, i + TITLE_LENGTH, person.getId(),
						getDataCellFormat(CellType.STRING_FORMULA));
				// 注释 start
				label = new Label(1, i + TITLE_LENGTH, person.getName(),
						getDataCellFormat(CellType.STRING_FORMULA));
				sheet.addCell(label);
				...
				if (person.getSex() >= 20) {
					insertOneCellData(
							...
							i + TITLE_LENGTH,
							person.getSex(),
							getDataCellFormat(CellType.NUMBER, "宋体", 10,
									Colour.YELLOW, Colour.YELLOW));
				} else {
					insertOneCellData(sheet, 2, i + TITLE_LENGTH,
							person.getSex(), getDataCellFormat(CellType.NUMBER));
				}
				insertOneCellData(sheet, 3, i + TITLE_LENGTH,
						person.getBirthday(), getDataCellFormat(CellType.DATE));
				// insertFormula(sheet, 5, 1, "C2+D2+E2",
				// getDataCellFormat(CellType.NUMBER_FORMULA));//SUM(C3:E3)
			}

			for (int i = 2; i < dataTitles.length; i++) {
				insertFormulaCell(sheet, i, data.size() + TITLE_LENGTH, 2,
						data.size() + TITLE_LENGTH,
						getDataCellFormat(CellType.NUMBER_FORMULA));
			}
			insertFormula(sheet, 4, data.size() + TITLE_LENGTH, "C6+D6",
					getDataCellFormat(CellType.NUMBER_FORMULA));// SUM(C3:E3)
	 ...
	 */
	private void insertFormulaCell(WritableSheet dataSheet, int col, int row,
	...
	private String getFormula(int col, int row, int startPos, int endPos)
		...
		if (col <= 25) {
			c1 = (char) (col % 26 + base);
			formula.append(c1).append(startPos).append(":").append(c1)
	 ...
	 * @return
	 */
	public WritableCellFormat getDataCellFormat(CellType type, String fontType,
			int fontSize, jxl.format.Colour fontColour,
		...
		try {
			...
			if (type == CellType.NUMBER || type == CellType.NUMBER_FORMULA) {// 数字
				...
				WritableFont wf = new WritableFont(
						WritableFont.createFont(fontType == null ? "宋体"
								: fontType), fontSize, WritableFont.NO_BOLD,
						false, jxl.format.UnderlineStyle.NO_UNDERLINE,
						fontColour == null ? jxl.format.Colour.BLACK
	...
	public static void main(String[] args) {
		...
		jxl.createExcelFile(filePath, "sheet1", titleData, "人员信息表",
				"D:/LogoPNG.png");
