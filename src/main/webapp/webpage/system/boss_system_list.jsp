...
//添加系统
function addSystem(){
	 window.location = "<%=basePath %>/web/bossSystem/addSystem";
}

//修改系统
function updateSystem(systemId){
	window.location = "<%=basePath %>/web/bossSystem/update?systemId="+systemId;
}

//禁用系统
function deleteSystem(systemId){
	$("#deleteSystemId").val(systemId);//将系统id 设置到隐藏域
	//打开弹窗
	var d = dialog({
		width:400,
		height:250,
...
//点击禁用确定按钮
function deleteBySystemId(){
	var systemId = $("#deleteSystemId").val();
	window.location = "<%=basePath %>/web/bossSystem/deleteSystem?systemId="+systemId;
}

//点击启用按钮
function qiyongSystem(systemId){
	$("#qiyongSystemId").val(systemId);//将系统id 设置到隐藏域
	//打开弹窗
	var d = dialog({
		width:400,
		height:250,
...
//点击禁用确定按钮
function qiyongBySystemId(){
	var systemId = $("#qiyongSystemId").val();
	window.location = "<%=basePath %>/web/bossSystem/qiyongSystem?systemId="+systemId;
}
...
<!--     <button class="btn btn-xs btn-success" onclick="addSystem()">
		...
		<h4 class="lighter text-muted">
             ...
             <div class="pull-right btn-bottom">
                 <a class="btn btn-xs btn-primary" onclick="addSystem()">添加系统</a>
                 <a class="btn btn-xs btn-danger">批量删除</a>
             </div>
         </h4>
					...
					<tr>
						...
						<td>
							<div class="hidden-sm hidden-xs btn-group">
								...
								<c:if test="${sys.systemStatus == 1}">
									<button class="btn btn-xs btn-danger" onclick="deleteSystem('${sys.systemId}')">
										禁用
									</button>
								</c:if>
								
								<c:if test="${sys.systemStatus == 0}">
									<button class="btn btn-xs btn-danger" onclick="qiyongSystem('${sys.systemId}')">
										启用
									</button>
								 </c:if>
