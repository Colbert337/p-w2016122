		...
		<div class="row">
			<div class="col-xs-12">
				...
				<div class="search-types">
					<div class="item">
						<label>运输公司编号:</label> <input type="text"
							name="sys_transportion_id" placeholder="输入运输公司编号" maxlength="10"
							value="${transportion.sys_transportion_id}" />
					</div>

					<div class="item">
						<label>运输公司名称:</label> <input type="text" name="transportion_name"
							placeholder="输入运输公司名称" maxlength="20"
							value="${transportion.transportion_name}" />
					</div>

					<div class="item">
						<label>运输公司状态:</label> <select class="chosen-select "
							name="status">
							<s:option flag="true" gcode="STATION_STATUS" form="transportion"
								field="status" />
						</select>
					</div>
					...
					<div class="item">
						<div class="input-daterange top" id="j-input-daterange-top">
							<label>平台有效期:</label> <input type="text" class=""
								name="expiry_date_after"
								value="${transportion.expiry_date_after}" readonly="readonly" />
							<span class=""> <i class="fa fa-exchange"></i>
							</span> <input type="text" class="" name="expiry_date_before"
								value="${transportion.expiry_date_before}" readonly="readonly" />
						</div>
					</div>

					<div class="item">
						<button class="btn btn-sm btn-primary" type="button"
							onclick="loadPage('#main','<%=basePath%>/webpage/poms/transportion/transportion_new.jsp');">
						...
						<thead>
							<tr>
								...
								<th onclick="orderBy(this,'transportion_name');commitForm();"
									id="transportion_name_order">运输公司名称</th>
								<th onclick="orderBy(this,'salesmen_name');commitForm();"
									id="salesmen_name_order">销售人员</th>
								<th onclick="orderBy(this,'operations_name');commitForm();"
									id="operations_name_order">运营人员</th>
								<th onclick="orderBy(this,'admin_username');commitForm();"
									id="admin_username_order">管理员账号</th>
								<th onclick="orderBy(this,'indu_com_number');commitForm();"
									id="indu_com_number_order">工商注册号</th>
								<th onclick="orderBy(this,'status');commitForm();"
									id="status_order">平台状态</th>
								<th onclick="orderBy(this,'address');commitForm();"
									id="address_order">注册地址</th>
								<th onclick="orderBy(this,'created_time');commitForm();"
									id="created_time_order" class="td-w2"><i id="created_time"
									class="ace-icon fa fa-clock-o bigger-110 hidden-480"></i>注册日期</th>
								<th onclick="orderBy(this,'expiry_date');commitForm();"
									id="expiry_date_order" class="td-w2"><i id="expiry_date"
									class="ace-icon fa fa-clock-o bigger-110 hidden-480"></i>平台有效期</th>
								<th onclick="orderBy(this,'address');commitForm();"
									id="address_order">账户余额</th>
						...
						<tbody>
							...
							<c:forEach items="${pageInfo.list}" var="list" varStatus="s">
								<tr id="${list.sys_transportion_id}">
									...
									<td><s:Code2Name mcode="${list.status}"
											gcode="STATION_STATUS"></s:Code2Name></td>
									<td>${list.address}</td>
									<%-- <td>${list.batch_no}</td>  --%>
									<td><fmt:formatDate value="${list.created_time}"
											type="both" /></td>
									<td><fmt:formatDate value="${list.expiry_date}"
											type="both" /></td>
									...
									<td class="text-center"><a class="option-btn-m"
										href="javascript:void(0);" title="修改" data-rel="tooltip">
											...
											onclick="preUpdate(this);"></i>
									</a> <a class="option-btn-m" href="javascript:void(0);"
										title="查看图片" data-rel="tooltip"> <i
											class="ace-icon fa fa-search-plus bigger-130"
											onclick="showInnerModel('${list.indu_com_certif}','${list.tax_certif}',$('#${list.sys_transportion_id}'));"></i>
									</a> <a href="javascript:void(0);" title="重置密码" data-rel="tooltip">
											<i class="ace-icon fa fa-key bigger-130"
											onclick="resetPassword(this);"></i>
									</a></td>
								</tr>
							</c:forEach>
						</tbody>
						...
						<nav>
							<ul id="ulhandle" class="pagination pull-right no-margin">
								<li id="previous"><a href="javascript:void(0);"
									aria-label="Previous" onclick="prepage('#formtransportion');">
										<span aria-hidden="true">上一页</span>
								</a></li>
								<li id="next"><a id="nexthandle" href="javascript:void(0);"
									aria-label="Next" onclick="nextpage('#formtransportion');">
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="shenhe-items-hd">运输公司信息</div>
				<table class="table">
					<tbody>
						<tr>
							<th width="15%">运输公司编号</th>
							<td><div id="sys_gas_station_id" name="show"></div></td>
							<th width="15%">运输公司名称</th>
				...
				<div class="row">
					<div class="col-sm-3">
						<ul class="ace-thumbnails clearfix">
							<li><a class="gastation-log-colorbox" href=""
								data-rel="colorbox"> <img class="img-responsive" src=""
									alt="" id="innerimg1">
