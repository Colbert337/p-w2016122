function changecouponkind(){
		...
		var  gas_station_Id = $("#gas_station").attr("sys_gas_station_id")	;
	if(coupon_kind=='1'){
		...
		$.ajax({
			...
			success: function(data){
				...
				$.each(gas_stationInfo, function(i, grstaion){
					if(gas_station_Id==grstaion.sys_gas_station_id){
						$("#gas_station").append("<option value='"+grstaion.sys_gas_station_id+"' selected='selected'>"+grstaion.gas_station_name+"</option>");
					}else{
...
function resetform(){
	loadPage('#main', '../webpage/poms/coupon/modifyCoupon.jsp');
}
